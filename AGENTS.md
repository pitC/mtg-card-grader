...
- **17Lands comparison**: First run fetches and caches to `localStorage`. Default CORS proxy is `api.cors.lol` (free, works from any origin). On non-dev hosts, set `?proxy=<working proxy>` or use `?proxy=direct` if the host allows CORS.
